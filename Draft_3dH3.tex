...
\section{\texorpdfstring{\EE}{E8(8)} ExFT and consistent truncations} \label{s:Review}

\subsection{Review of \texorpdfstring{\EE}{E8(8)} exceptional field theory}
...
\subsection{Generalized Scherk-Schwarz ansatz and consistent truncations}
The \EE exceptional field theory is well suited to the construction of consistent truncation of type~II and 11d supergravities to three-dimensional maximal supergravity. These truncations arise as generalised Scherk-Schwarz reduction, described by an \EE-valued twist matrix $U_{\fl{M}}{}^{M}$ and a scale factor $\rho$. The compactification Ansätze are as follows~\cite{Galli:2022idq}:
...
The constant tensor $X_{\fl{M}\fl{N}}{}^{\fl{P}}$ plays the role of the embedding tensor of the three-dimensional gauged supergravity. It has the following expression:
\begin{equation}
	\begin{aligned}
		X_{\bar M\bar N}{}^{\bar P}&=-\rho^{-1}\,\Gamma_{\bar M\bar N}{}^{\bar P} + \rho^{-1}\,f^{\bar P}{}_{\bar N\bar Q}f^{\bar Q\bar K}{}_{\bar L}\,\Gamma_{\bar K\bar M}{}^{\bar L}-\frac{1}{60}\,\rho^{-1}\,f^{\bar P\bar K}{}_{\bar N}f_{\bar M\bar L}{}^{\bar Q}\,\Gamma_{\bar K\bar Q}{}^{\bar L}\\
		&\quad-\frac{1}{2}\,\rho^{-1}\,f^{\bar P}{}_{\bar N\bar Q}f^{\bar Q\bar K}{}_{\bar M}\,\Gamma_{\bar R\bar K}{}^{\bar R}+\left(\delta_{\bar M}{}^{\bar K}\delta_{\bar N}{}^{\bar P}-\frac{1}{2}\,f_{\bar M}{}^{\bar L\bar K}f_{\bar N\bar L}{}^{\bar P}\right)\,\xi_{\bar K}.
	\end{aligned}
\end{equation}
Here, we defined the ${\rm E}_{8(8)}$ current $\Gamma_{\bar M\bar N}{}^{\bar P}=U_{\bar M}{}^{K}U_{\bar N}{}^{L}\partial_{K}U^{\bar P}{}_{L}$ and the trombone gauging
\begin{equation}
	\xi_{\bar M} = 2\,U_{\bar M}{}^{N}\partial_{N}\rho^{-1}+\rho^{-1}\,\partial_{N}U_{\bar M}{}^{N}.
\end{equation}
In the following, we consider $\xi_{\bar M}=0$. The embedding tensor is most nicely expressed once projected on the adjoint representation
\begin{equation}
	X_{\bar M\bar N} = -\frac{1}{60}\,X_{\bar M\bar P}{}^{\bar Q}\,f_{\bar N\bar Q}{}^{\bar P} = 2\,\rho^{-1}\,\Gamma_{(\bar M\bar N)} - \rho^{-1}\,\Gamma_{\bar K(\bar M}{}^{\bar L}\,f_{\bar N)\bar L}{}^{\bar K},
\end{equation}
with the projection of the current $\Gamma_{\bar M\bar N} = (1/60)\,\Gamma_{\bar M\bar P}{}^{\bar Q}\,f_{\bar N\bar Q}{}^{\bar P}$.
...
\begin{equation}
	\begin{aligned}
		S_{\rm ExFT} \underset{\rm gSS}{=} \int\d^{248}Y\,\rho^{-1}\int \d^{3}x & \bigg[\sqrt{\vert\mathring{g}\vert}\bigg(\mathring{R}+\frac{1}{240}\,{\cal D}_{\mu}\M_{\bar M\bar N}{\cal D}^{\mu}\M^{\bar M\bar N}-V\bigg) \\
		&-\varepsilon^{\mu\nu\rho}\,X_{\bar M\bar N}\,\A_{\mu}{}^{\bar M} \left(\partial_{\nu}\A_{\rho}{}^{\bar N}-\frac{1}{3}\,\llbracket\A_{\nu},\A_{\rho}\rrbracket^{\bar N}\right)\bigg],
	\end{aligned}
\end{equation}
...
\begin{equation}
	V = X_{\bar M\bar N}\,X_{\bar P\bar Q}\left(\frac{1}{28}\,\M^{\bar M\bar P}\M^{\bar N\bar Q}+\frac{1}{2}\,\M^{\bar M\bar P}\eta^{\bar N\bar Q}+\frac{13}{28}\,\eta^{\bar M\bar P}\eta^{\bar N\bar Q}\right).
\end{equation}
