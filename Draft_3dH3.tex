...
\begin{equation}
	\alpha = \frac{1}{28}\,,\quad \beta = \frac{1}{2}\,,\quad a=1\,,\quad \delta=\frac{13}{28} \,. %, \quad \gamma=0.
\end{equation}
...
\end{equation}
This solution of the section condition defines the ``geometric'' $\SL{3}_1 \times \mathbb{R}^+_1 \subset \SL{4}_1$ and $\SL{3}_2 \times \mathbb{R}^+_2 \subset \SL{4}_2$ subgroups, that will play an important role soon.

We can now use one copy of the $\SL{4}$ frame \eqref{eq:SLnFrame} for each $\SL{4}$ subgroup of \eqref{eq:SL4Embedding} to construct a generalised parallelisation for the full $\En{7}$ ExFT, with an embedding tensor via \eqref{eq:GenLeibniz} \EM{given by \ldots}.

\EM{Include breaking of adjoint here?}\\
\EM{Should mention the geometric $\SL{3}$ subgroups here???}

\EM{What we should actually do: Read this and next section again and make sure they make sense. Maybe when referring to the $\SL{3}$ subgroups in the next section, we can cite \eqref{eq:PhysicalCoords}}
...
\begin{equation} \label{eq:embeddingtensor@origin}
	...
	\end{aligned}
\end{equation}
With this embedding tensor, the potential~\eqref{eq:3dpotential} is stabilized at the scalar origin for $\lambda=4\sqrt{3}\,\sqrt{1+(1/\alpha^{2})}$. We fix this value in the following.
...
\paragraph{}
The 3d solution preserves ${\cal N}=(4,4)$ supercharges, with isometry group ${\cal G}=D^{1}(2,1;\alpha)_{\rm L}\times D^{1}(2,1;\alpha)_{\rm R}$, \textit{i.e.} two copies of the large ${\cal N}=4$ supergroup. The even part of $D^{1}(2,1;\alpha)$ is isomorphic to ${\rm SL}(2,\mathbb{R})\times{\rm SO}(3)\times\ts{{\rm SO}(3)}$ and the bosonic isometries of the ${\rm AdS}_{3}\times S^{3}\times\ts{S}^{3}$ background are built from the even part of ${\cal G}$. The ${\rm AdS}_{3}$ isometry group is ${\rm SO}(2,2)\simeq{\rm SL}(2,\mathbb{R})_{\rm L}\times{\rm SL}(2,\mathbb{R})_{\rm R}$, and the factors ${\rm SO}(3)_{\rm L}\times{\rm SO}(3)_{\rm R}\times\ts{{\rm SO}(3)}_{\rm L}\times\ts{{\rm SO}(3)}_{\rm R}\simeq{\rm SO}(4)\times\ts{SO(4)}$ combine into the isometry groups of the two spheres. The spectrum organizes into multiplets of ${\cal G}$, made out of products of long representations $[\ell,\ts{\ell}]$ of $D^{1}(2,1;\alpha)$ (see app.~\ref{app:d21a} for a review of the representations of $D^{1}(2,1;\alpha)$). Explicitly, the spectrum is~\cite{Eberhardt:2017fsi}
...
We identified a 11-parameter family of solutions, connected to the origin and given by the \EE coset representative \CE{($\fl{28}\leftrightarrow28$ compared to the code, I think we made a mistake and confused down and up indices)}
\begin{equation}
	\begin{aligned}
		...
		& \times \exp\bigg[\Omega\Big(\big(t_{(\mathbf{63},\mathbf{1})}\big)_{1}{}^{1}-\big(t_{(\mathbf{63},\mathbf{1})}\big)_{2}{}^{2}-\big(t_{(\mathbf{63},\mathbf{1})}\big)_{3}{}^{3}+\big(t_{(\mathbf{63},\mathbf{1})}\big)_{8}{}^{8}\Big)\\
		&\ \qquad+\ts{\Omega}\Big(\big(t_{(\mathbf{63},\mathbf{1})}\big)_{4}{}^{4}-\big(t_{(\mathbf{63},\mathbf{1})}\big)_{5}{}^{5}-\big(t_{(\mathbf{63},\mathbf{1})}\big)_{6}{}^{6}+\big(t_{(\mathbf{63},\mathbf{1})}\big)_{7}{}^{7}\Big)\bigg] \\
		& \times \exp\left[\ln(T)\left(\frac{1}{120}\,\big(t_{(\mathbf{63},\mathbf{1})}\big)_{I}{}^{I}-\frac{1}{120}\,\big(t_{(\mathbf{63},\mathbf{1})}\big)_{A}{}^{A}-\frac{1}{2}\,\big(t_{(\mathbf{1},\mathbf{3})}\big)_{1}\right)\right],
	\end{aligned}
\end{equation}
where we denoted the generators according to the decomposition~\eqref{eq:248decompoinSL8}. The $(\chi_{i},\ts{\chi}_{i},\zeta_{i},\ts{\zeta}_{i})$ deformations excite the $(\mathbf{6},\mathbf{1})_{2}$ and $(\mathbf{1},\mathbf{6})_{2}$ generators in the branching~\eqref{eq:56inSL4SL4}. \CE{(Describe what we do for $T$? I think there is some freedom in the coeff there.)} \CE{(Describe what we do for $\Omega$? Check coeff with code. Possible to change the signs to $(+,+,-,-)$, but very long to compute uplift.)}
...
\paragraph{\boldmath $(\chi, \ts{\chi}, T)$}
...
\begin{equation}
	\begin{aligned}
		...
		&\quad +\d\ts{\theta}^{2} + \cos^{2}(\ts{\theta})\,\d\ts{\phi}'_{1}{}^{2} + \sin^{2}(\ts{\theta})\,\d\ts{\phi}'_{2}{}^{2} + T^{2}\,\d z^{2}, \\[5pt]
...
This deformation breaks $\SO{3}_{\rm L}\times\SO{3}_{\rm R}\times\ts{\SO{3}}_{\rm L}\times\ts{\SO{3}}_{\rm R}$ to ${\rm U}(1)_{\rm L}\times{\rm U}(1)_{\rm R}\times\ts{{\rm U}(1)}_{\rm L}\times\ts{{\rm U}(1)}_{\rm R}$ and all supersymmetries. Concerning the spectrum, the conformal dimension of each physical field (\textit{c.f.} eq.~\eqref{eq:confdims3s3s1}) get shifted by replacing
\begin{equation} \label{eq:confdimshiftchi}
	2\pi n\longrightarrow \frac{2\pi n}{T} + \frac{1}{2T}\Big((q_{\rm L}+q_{\rm R})\,\chi_{1}+(q_{\rm L}-q_{\rm R})\,\chi_{2}+ \alpha\,(\ts{q}_{\rm L}+\ts{q}_{\rm R})\,\ts{\chi}_{1}+ \alpha\,(\ts{q}_{\rm R}-\ts{q}_{\rm L})\,\ts{\chi}_{2}\Big),
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\left(\pm1,\pm1;0,0\right):\quad \Delta=\frac{1}{2}+\sqrt{1+\frac{\left(\chi_{1}+\chi_{2}+\alpha\,\big(\ts{\chi}_{1}-\ts{\chi}_{2}\right)^{2}\big)}{4\,T^{2}(1+\alpha^{2})}}, \\
		\left(\pm1,\mp1;0,0\right):\quad \Delta=\frac{1}{2}+\sqrt{1+\frac{\left(\chi_{1}+\chi_{2}-\alpha\,\big(\ts{\chi}_{1}-\ts{\chi}_{2}\right)^{2}\big)}{4\,T^{2}(1+\alpha^{2})}}, \\
		\left(0,0;\pm1,\pm1\right):\quad \Delta=\frac{1}{2}+\sqrt{1+\frac{\left(\chi_{1}-\chi_{2}+\alpha\,\big(\ts{\chi}_{1}+\ts{\chi}_{2}\right)^{2}\big)}{4\,T^{2}(1+\alpha^{2})}}, \\
		\left(0,0;\pm1,\mp1\right):\quad \Delta=\frac{1}{2}+\sqrt{1+\frac{\left(\chi_{1}-\chi_{2}+\alpha\,\big(\ts{\chi}_{1}+\ts{\chi}_{2}\right)^{2}\big)}{4\,T^{2}(1+\alpha^{2})}}.
	\end{aligned}
\end{equation}
SUSY enhancement points are then given by combinations of the parameters that leave some modes invariant with $\Delta=\nicefrac{3}{2}$. The different possibilities are listed below. \CE{(Shall we give the multiplets? At least for the truncation?)}

\begin{itemize}
	\item {\boldmath ${\cal N}=2$} The four-dimensional hypersurfaces
	\begin{equation}
		\chi_{2} = \chi_{1} \pm\alpha\,(\ts{\chi}_{1}+\ts{\chi}_{2}) \quad {\rm or}\quad \chi_{2} = -\chi_{1} \pm\alpha\,(\ts{\chi}_{1}-\ts{\chi}_{2}),
	\end{equation}
	give enhancements to ${\cal N}=(2,0)$ and ${\cal N}=(0,2)$, respectively. The spectrum then organizes into multiplets of $\SU{1\vert1,1}$.
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	leads to enhancements to ${\cal N}=(4,0)$ and ${\cal N}=(0,4)$, respectively. On these three-dimensional hyper-surfaces, the isometry group is $\SO{4}\times {\rm U}(1)^{2}$ and the spectrum reorganizes into multiplets of $D^{1}(2,1;\alpha)$. Another possibility is to set
	\begin{equation}
		\begin{cases}
...
\paragraph{\boldmath $(\zeta,\ts{\zeta},T)$}
...
\begin{equation}
	\begin{aligned}
		...
		&+\Delta\,T^{2}\,\cos^{2}(\theta)\cos^{2}(\ts{\theta}) \left(\alpha\,\ts{\zeta}_{2}\,\d\phi_{1}-\zeta_{2}\,\d\ts{\phi}_{1}\right)^{2}+\Delta\,T^{2}\,\sin^{2}(\theta)\sin^{2}(\ts{\theta}) \left(\alpha\,\ts{\zeta}_{1}\,\d\phi_{2}-\zeta_{1}\,\d\ts{\phi}_{2}\right)^{2}\\
		&+\Delta\,T^{2}\,\cos^{2}(\theta)\sin^{2}(\ts{\theta}) \left(\alpha\,\ts{\zeta}_{1}\,\d\phi_{1}-\zeta_{2}\,\d\ts{\phi}_{2}\right)^{2}+\Delta\,T^{2}\,\sin^{2}(\theta)\cos^{2}(\ts{\theta}) \left(\alpha\,\ts{\zeta}_{2}\,\d\phi_{2}-\zeta_{1}\,\d\ts{\phi}_{1}\right)^{2}\\
		&-2\,\alpha\,\Delta\,T^{2}\left(\zeta_{1}\,\cos^{2}(\theta)\,\d\phi_{1}+\zeta_{2}\,\sin^{2}(\theta)\,\d\phi_{2}\right)\left(\ts{\zeta}_{1}\,\cos^{2}(\ts{\theta})\,\d\ts{\phi}_{1}+\ts{\zeta}_{2}\,\sin^{2}(\ts{\theta})\,\d\ts{\phi}_{2}\right)\\
		&+\Delta\,T^{2}\,\cos^{2}(\theta)\left(\alpha\,\zeta_{1}\,\d\phi_{1}+\d z\right)^{2}+\Delta\,T^{2}\,\sin^{2}(\theta)\left(\alpha\,\zeta_{2}\,\d\phi_{2}+\d z\right)^{2}\\
		&+\Delta\,T^{2}\,\cos^{2}(\ts{\theta})\left(\ts{\zeta}_{1}\,\d\ts{\phi}_{1}-\d z\right)^{2}+\Delta\,T^{2}\,\sin^{2}(\ts{\theta})\left(\ts{\zeta}_{2}\,\d\ts{\phi}_{2}-\d z\right)^{2},
	\end{aligned}
\end{equation}
with the warp factor
\begin{equation}
	\Delta^{-1} = 1+T^{2}\left(\zeta_{1}^{2}\sin^{2}(\theta)+\zeta_{2}^{2}\cos^{2}(\theta)+\ts{\zeta}_{1}^{2}\sin^{2}(\ts{\theta})+\ts{\zeta}_{2}^{2}\cos^{2}(\ts{\theta})\right).
\end{equation}.
