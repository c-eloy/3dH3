...
\begin{equation}
	\cU_{\fl{M}}{}^M \longrightarrow \cU'_{\fl{M}}{}^M = \cU_{\fl{M}}{}^N \exp(C)_N{}^M \,,
\end{equation}
with $C_M{}^N = \lambda\, z\,\rho^{-1}\, t_{1238\,M}{}^N$ and $\lambda$ a numerical parameter corresponding to the amount of 7-form flux. \EM{The resulting embedding tensor is \ldots}
...
Pure NSNS solution, after \cite{deBoer:1999gea}
\begin{equation}
	\begin{aligned}
		\d s^{2} &= \ell_{\rm AdS}^{2}\,\d s^{2}\left({\rm AdS}_{3}\right) + \alpha^{2}\,\d s^{2}\big(S^{3}\big)+\d s^{2}\big(\ts{S}^{3}\big)+\d\psi^{2},\\
...
\begin{equation}
	\begin{aligned}
		\d s^{2} &= \ell_{\rm AdS}^{2} \,\d s^{2}({\rm AdS}_{3}) +\alpha^{2}\left(\d\theta^{2} + \cos^{2}(\theta)\,\kappa_{1}^{2} + \sin^{2}(\theta)\,\kappa_{2}^{2} \right) \\
		&\quad +\d\ts{\theta}^{2} + \cos^{2}(\ts{\theta})\,\ts{\kappa}_{1}^{2} + \sin^{2}(\ts{\theta})\,\ts{\kappa}_{2}^{2} + \d\psi^{2}, \\[5pt]
		H_{(3)} &= 2\,\ell_{\rm AdS}^{2}\,{\rm Vol}({\rm AdS_{3}}) + 2\,\alpha^{2}\cos(\theta)\sin(\theta)\,\d\theta\wedge\kappa_{1}\wedge\kappa_{2}+ 2\cos(\ts{\theta})\sin(\ts{\theta})\,\d\ts{\theta}\wedge\ts{\kappa}_{1}\wedge\ts{\kappa}_{2},
	\end{aligned}
\end{equation}
where
\begin{equation}
	\begin{aligned}
		\kappa_{1} &= \d\phi_{1} + \chi_{1}\,\d\psi,\\
		\kappa_{2} &= \d\phi_{2} - \chi_{2}\,\d\psi,\\
		\ts{\kappa}_{1} &= \d\ts{\phi}_{1} + \ts{\chi}_{1}\,\d\psi,\\
		\ts{\kappa}_{2} &= \d\ts{\phi}_{2} - \ts{\chi}_{2}\,\d\psi.
	\end{aligned}
\end{equation}
