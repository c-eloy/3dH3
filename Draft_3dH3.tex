...
\begin{equation} \label{eq:embeddingtensor@origin}
	\begin{aligned}
		...
		\begin{cases}
			\displaystyle X_{\fl{I}}{}^{\fl{J}}{}_{;\fl{KL}+}=-\frac{1}{\alpha}\,\delta_{\fl{I[K}}\delta_{\fl{L]}}{}^{\fl{J}},\\[7pt]
		...
		\begin{cases}
			\displaystyle X_{\fl{A}}{}^{\fl{B}}{}_{;}{}^{\fl{CD}}{}_{+}=-\,\delta_{\fl{A}}{}^{\fl{[C}}\delta^{\fl{D]B}},\\[7pt]
			\displaystyle X_{\fl{A}}{}^{\fl{I}}{}_{;}{}^{\fl{BJ}}{}_{+}=-\frac{1}{2}\,\delta_{\fl{A}}{}^{\fl{B}}\delta^{\fl{IJ}},\\
		\end{cases} \\[5pt]
		X_{(\mathbf{70},\mathbf{1});(\mathbf{1},\mathbf{3})}:&\  X_{\fl{IJKL};\fl{\alpha}} = -\frac{\lambda}{12\sqrt{6}}\,\varepsilon_{\fl{IJKL}}\,\delta_{\fl{\alpha}2},
	\end{aligned}
\end{equation}
with the component $\fl{\alpha} = 2$ of the adjoint of ${\rm SL}(2)$ corresponding to the generator $e = \begin{pmatrix} 0 & 1 \\ 0 & 0\end{pmatrix}$ in the basis $\fl{i}=\{+,-\}$. With this embedding tensor, the potential~\eqref{eq:3dpotential} is stabilized at the scalar origin for $\lambda=4\sqrt{3}\,\sqrt{1+(1/\alpha^{2})}$. We fix this value in the following.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
with $H_{(3)} = \d B_{(2)}$ and \CE{(with the normalization in eq.~\eqref{eq:3dpotential} and $\ell_{\rm AdS}^{2}=2/V_{0}$, the code gives an additional factor $1/2$)}
\begin{equation}
	\ell_{\rm AdS}^{2} = \frac{\alpha^{2}}{1+\alpha^{2}}.
\end{equation}
The $S^{1}$ coordinate $z$ has periodicity $z\rightarrow z+1$. \CE{(add $S^{1}$ modulus here?)}
...
This deformation breaks $\SO{3}_{\rm L}\times\SO{3}_{\rm R}\times\ts{\SO{3}}_{\rm L}\times\ts{\SO{3}}_{\rm R}$ to ${\rm U}(1)_{\rm L}\times{\rm U}(1)_{\rm R}\times\ts{{\rm U}(1)}_{\rm L}\times\ts{{\rm U}(1)}_{\rm R}$ and all supersymmetries. Concerning the spectrum, the conformal dimension of each field (\textit{c.f.} eq.~\eqref{eq:confdims3s3s1}) get shifted by replacing
\begin{equation}
	2\pi n\longrightarrow \frac{2\pi n}{T} + \frac{1}{2T}\Big((q_{\rm R}+q_{\rm L})\,\chi_{1}+(q_{\rm R}-q_{\rm L})\,\chi_{2}+ \alpha\,(\ts{q}_{\rm R}+\ts{q}_{\rm L})\,\ts{\chi}_{1}+ \alpha\,(\ts{q}_{\rm L}-\ts{q}_{\rm R})\,\ts{\chi}_{2}\Big),
\end{equation}
where the $q$ denote the charges under the different ${\rm U}(1)$. \CE{(Issue here!)}

\CE{(Discuss SUSY enhancement)}
...
\begin{equation}
	...
	\end{array}
\end{equation}
with $h_{0}=(\alpha\,\ell+\ts{\ell})/(1+\alpha)$.\footnote{Shortenings occur for $\ell<1$ or $\ts{\ell}<1$, see ref.~\cite{deBoer:1999gea}. \CE{(Add them here?)}} Two short multiplets can combine into a long multiplets $[\ell,\ts{\ell}]$ as follows:
\begin{equation}
